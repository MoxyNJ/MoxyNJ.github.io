...
function BlogListPage(props) {
  ...
  const InfoCard = () => {
      ...
      .map(([category, tagsForCategory]) => (
        <div key={category} style={{ display: "flex", flexWrap: "wrap" }}>
          {tagsForCategory.map((tag, index) => (
            <Link
              className={`post__tags margin-right--sm margin-bottom--sm`}
    ...
    return (
      <div
        className={
          viewType === "card" ? `col col--3 ${styles["info-wrapper"]}` : ""
        }
        style={{
          display: `${viewType === "card" && isBlogPage ? "" : "none"}`,
        }}
      >
          ...
          <div className={`bloghome__posts-card ${styles["info-wrapper"]}`}>
            <div className={`row ${styles.card}`}>
              <div className={styles["personal-info-wrapper"]}>
                <img
                  className={styles["personal-img"]}
                  src="/img/ninjee-icon.png"
                  alt="logo"
                ></img>
                <h3 className={styles["personal-name"]}>ninjee</h3>
              ...
              {!isPaginated && (
                <h1 className="blog__section_title" id="homepage_blogs">
                  <Translate description="latest blogs heading">
                    {!metadata.permalink.includes("essay")
                      ? "最新博客"
                      : "个人随笔"}
                  </Translate>
                  &nbsp;
                      ...
                      return (
                        <React.Fragment key={blogMetaData.permalink}>
                          ...
                          >
                            {sticky && (
                              <div
                                className={`post__list-stick iconfont`}
                              ></div>
                            )}
