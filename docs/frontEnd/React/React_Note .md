...
- 执行函数组件 useLayoutEffect 的销毁函数，useLayoutEffect 是 **同步执行** 的。
- 调用 class 组件 componentWillUnmount 生命周期函数。
...
- 根据 current 状态，调用 class 生命周期：compinentDidMount 或 ComponentDidUpdate。
...
![截屏2022-08-07 22.42.10](images/React_Note%20.assets/%E6%88%AA%E5%B1%8F2022-08-07%2022.42.10.png)
...
同级的
 ...
 .alternate 属性，复用当前 current 树上的 fiber 节点。RootFiber 已经创建不需要复制，但其子节点由于尚不存在， 则 react 需要重新创建一份，和 current 树上的 fiber 建立起 alternate 关联。渲染完毕后，workInProgresss 再次变成 current 树。

![截屏2022-08-07 22.49.01](images/React_Note%20.assets/%E6%88%AA%E5%B1%8F2022-08-07%2022.49.01.png)
