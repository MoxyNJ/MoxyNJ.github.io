...
事实上，隐式绑定利用了函数调用的规则（见 2.）：`func.call(obj, p1, p2, ..)`
...
-   作用域关系在代码的编译阶段已经确定，也就是说，作用域关系是静态绑定的，直接看代码分析的，而不考虑代码的执行。**（见问题 4 的双红色叹号）**
...
❗️❗️ 即使下面的 Person 是通过 new 调用的。但 foo2 箭头函数的上级作用域依然是 Person 函数作用域。
